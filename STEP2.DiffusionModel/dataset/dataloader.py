...
class UniformCacheDataset(CacheDataset):
    ...
    def data_statis(self):
        ...
        for img in self.data:
            key = get_key(img['name'])
            # print(f"datasetkey: {args.datasetkey}")
...
def get_key(name):
    # 使用正则表达式来提取类似 "BraTS-GLI" 的标识符
    match = re.match(r"([A-Za-z]+-[A-Za-z]+)-\d{5}-\d{3}", name)
    if match:
        # 返回提取到的模板或类别标识符
...
def load_config(config_path):
    ...
    with open(config_path, 'r') as f:
        config_dict = yaml.safe_load(f)  # 加载 YAML 文件
    return dict_to_namespace(config_dict)  # 转换为 Namespace 对象

if __name__ == "__main__":
    # 加载 YAML 配置文件
    config = load_config('/home/data/yanghao/DiffTumor-main/DiffTumor-main/STEP2.DiffusionModel/config/dataset/synt_ct.yaml')

    # 将配置传递给 get_loader 函数
    train_dataloader, train_sampler, dataset_size = get_loader(config)  # 传入加载的配置参数
